#define CONTROLLER_SERIAL
  ...
  #include <ESP32Controller_ESPNOW.h>
  Config_ESPNOW config;

#elif defined(CONTROLLER_BLUETOOTH)
  #include <ESP32Controller_BluetoothSerial.h>
...
ESP32Controller<BlinkCommand> controller(config,command);
...
void loop() {
  if (controller.update()) {
    if (!controller.get_input().is_on) {
      ledcWrite(pin_led, 0);
      //ledcWrite(0, 0);
    }else {
      ledcWrite(pin_led, controller.get_input().power);
      //ledcWrite(0, controller.get_input().power);
    }
    Serial.print(controller.get_input().is_on);
    Serial.print(",");
    Serial.print(controller.get_input().power);
    Serial.println();
  } else {
    ledcWrite(pin_led, 0);
