...
step4. 設定用の構造体(Config_~~)を実体化  
step5. ``Controller<構造体の型名> コントローラーオブジェクト(構造体の実体名);``で宣言  
step6. setup関数内で``コントローラーオブジェクト.begin();``で初期化  
step7. loop関数内で``コントローラーオブジェクト.update();``で値の更新  
step8. ``コントローラーオブジェクト.get_input().やり取りしたい変数名``で値を取得  
